...
export default function ReportItemPage() {
  ...
  // Analyze image for labels/tags only
  const analyzeImage = async (uri: string) => {
    try {
      ...
      if (labels?.length) setTags(labels);
    } catch (err: any) {
  ...
  const getFloors = () => {
    if (building === "KA") return Object.keys(kaData.KA);
    if (building === "KB") return Object.keys(kbData.KB);
    return [];
  };

  const getLocations = () => {
    if (building === "KA" && floor)
      return (kaData.KA as Record<string, string[]>)[floor] || [];
    if (building === "KB" && floor)
      return (kbData.KB as Record<string, string[]>)[floor] || [];
    return [];
  };
  ...
  return (
    <SafeAreaView style={{ flex: 1 }}>
      ...
      >
        <Text style={styles.title}>Upload Lost/Found Item</Text>
